                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="card-text">
                            ...
                            <a href="/participant/trip-details/<?= $trip['trip_id']; ?>" class="btn btn-info btn-sm">View Details</a>

                            <span class="badge bg-<?php echo ($trip['status'] == 'accepted') ? 'success' : (($trip['status'] == 'declined') ? 'danger' : 'secondary'); ?>">
                                <?= htmlspecialchars($trip['status'] ?? 'Pending') ?>
                            </span>
                            <br>
                            <?php if (!empty($trip['responded_at'])): ?>
                            <small class="text-muted">Responded at: <?= htmlspecialchars($trip['responded_at']) ?></small>
                            <?php endif; ?>
                        </p>

                        <!-- Only show Accept and Decline buttons if status is pending -->
                        <?php if ($trip['status'] == 'pending'): ?>
                            <form method="POST" action="/participant/update-status">
                                <input type="hidden" name="trip_id" value="<?= $trip['trip_id']; ?>">
                                <button type="submit" name="status" value="accepted" class="btn btn-success btn-sm me-2">Accept</button>
